...
//DEPRECATED - DELETE THIS WIDGET WHEN NOT IN USE
class estore_child_woocommerce_product_carousel extends WP_Widget {
	function __construct() {
		...
		parent::__construct( false, $name = esc_html__( 'A-Child: Products Carousel', 'estore' ), $widget_ops, $control_ops);
	}
		...
		<div class="tg-container">
			...
			<div class="featured-wrapper clearfix">
				...
				while ($featured_query->have_posts()) :
					...
					<li>
					<?php
						$image_url = wp_get_attachment_image_src(get_post_thumbnail_id(),'shop_single', false); ?>
...
class estore_child_woocommerce_product_grid extends WP_Widget {
	function __construct() {
		...
		parent::__construct( false, $name = esc_html__( 'A-Child: Product Grid', 'estore' ), $widget_ops, $control_ops);
	}
			...
			<div class="collection-block-wrapper tg-column-wrapper clearfix">
						...
						<div class="hot-product-content-wrapper clearfix">
							...
							<figure class="hot-img">
								<a href="<?php the_permalink(); ?>">
									...
									<?php } else { ?>
										<img data-src="<?php echo get_template_directory_uri() . '/images/placeholder-shop-380x250.jpg'; ?>" src="<?php echo( get_stylesheet_directory_uri() . '/none.jpg') ?>" alt="<?php the_title_attribute(); ?>" width="250" height="180" >
									<?php } ?>
								</a>
				...
				<?php }
					if($count > 1 && $count < 7 || $count > 6) { ?>
						...
						<?php
							$image_url = wp_get_attachment_image_src(get_post_thumbnail_id(),'shop_thumbnail', false); ?>
							<figure class="product-list-img">
								<a href="<?php the_permalink(); ?>">
									<?php if($image_url[0]) { ?>
									<img data-src="<?php echo esc_url( $image_url[0] ); ?>" src="<?php echo( get_stylesheet_directory_uri() . '/none.jpg') ?>" alt="<?php the_title_attribute(); ?>">
									<?php } else { ?>
